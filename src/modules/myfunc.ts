function getSeries(name: string) {
    ...
    else if (name.match("IJCNN")) { series = "IJCNN"; }
    else if (name.match("IJCV")) { series = "IJCV"; }
    else if (name.match("IS")) { series = "IS"; }
    else if (name.match("JMLR")) { series = "JMLR"; }
    ...
    else if (name.match("SPIC")) { series = "SPIC"; }
    ...
    else if (name.match("MICCAI")) { series = "MICCAI"; }
    else if (name.match("ACCV")) { series = "ACCV"; }
    else if (name.match(/.*Int.*Conf.*3D Vision.*/i)) { series = "3DV"; }
    else if (name.match(/.*3D.*object.*retrieval.*/i)) { series = "3Dor"; }
    ...
    else if (name.match(/.*Computer.*graphics.*forum.*/i)) { series = "CGF"; }
    ...
    else if (name.match(/.*Int.*Conf.*Robot.*system.*/i)) { series = "IROS"; }
    else if (name.match(/.*Int.*Conf.*Robot.*Automation.*/i)) { series = "ICRA"; }
    ...
    else if (name.match(/.*Int.*Joint.*Conf.*Neural.*Network.*/i)) { series = "IJCNN"; }
    ...
    else if (name.match(/.*Signal.*Process.*Image.*Communication.*/i)) { series = "SPIC"; }
    else if (name.match(/.*Signal.*Process.*/i)) { series = "SP"; }
    else if (name.match(/Journal of Machine Learning Research/i)) { series = "JMLR"; }
    else if (name.match(/Medical Image Computing and Computer.*Assisted Intervention/i)) { series = "MICCAI"; }
    else if (name.match(/Asian Conference on Computer Vision/i)) { series = "ACCV"; }
    else if (name.match(/.*arXiv.*/i)) { series = "arXiv"; }
    return series;
}
export class myHelper {
    ...
    static update(items) {        
        for (let i = 0; i < items.length; i++) {
            let name = "";
            if (items[i].getField('conferenceName')) { name = items[i].getField('conferenceName') }
            else if (items[i].getField('publicationTitle')) { name = items[i].getField('publicationTitle') }
            else if (items[i].getField('proceedingsTitle')) { name = items[i].getField('proceedingsTitle') }
            else if (items[i].getField('repository')) { name = items[i].getField('repository') }

            let series = "";
            if (items[i].getField('series')) { series = items[i].getField('series') }
            else {
